  ...
  if (isLoading) {
    return (
      <div className="flex min-h-svh items-center justify-center p-6">
        <p className="text-muted-foreground animate-pulse">Loading counter...</p>
      </div>
    );
  }
    ...
    <div className="flex min-h-svh items-center justify-center p-6 bg-background">
      <div className="flex w-full max-w-sm flex-col gap-6 rounded-lg border bg-card p-6 shadow-sm">
        ...
        <div className="flex items-center justify-center gap-4">
          <Button variant="outline" size="lg" onClick={decrement} className="flex-1 text-2xl" aria-label="Decrease counter">
            -
          </Button>
          <Button size="lg" onClick={increment} className="flex-1 text-2xl" aria-label="Increase counter">
            +
          </Button>
        </div>
      </div>
